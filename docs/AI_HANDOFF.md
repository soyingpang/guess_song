...
- 手機端已改為入房前選「在現場 / 不在現場」：`player.html` 有 `#onsiteModeButton` / `#remoteModeButton` 和 `#phoneRemotePanel`；入房後 `player.js` 會鎖住模式，避免玩家中途切換。不在現場模式會顯示同步倒數、A/B 組分、開咪名單、玩家狀態，並用 `#phoneRemotePlayerHost` 建立被遮罩的同步播放器；手機瀏覽器如擋 YouTube 有聲自動播放，可按「開聲並同步」，若仍未聽到聲音，再點一下遮罩播放器區域，讓觸控直接落到 YouTube iframe。
