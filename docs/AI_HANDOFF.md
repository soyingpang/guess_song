...
- 「前台預備」不可直接露出 YouTube 縮圖 / 歌名。`display.js` 會在 frontReady 且未播放時加 `stage-mask.is-prep-cover`，`styles.css` 只留右下角小窗給主持處理廣告，其餘畫面用實色遮住。
...
- 手機四選一選項只應在正式播放時顯示；預備階段 `buildPlayerState()` 不送選項，`handleChoiceAnswer()` 亦會拒絕未正式播放時的答案。
