...
主持人先開後台，再按「開前台」。前台右下角會出現 QR code。玩家用手機掃碼後，輸入名字，再選「在現場 / 不在現場」加入遊戲；A/B 組會由系統按人數自動平均分配，主持仍可在後台手動調整。
...
未配置 Firebase 時，多人連線仍使用原本瀏覽器 WebRTC / PeerJS 後備；主持人後台要保持開住，因為分數同房間狀態由主持人頁面管理。
