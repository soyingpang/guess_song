...
- 主持頁可按「排行榜」，玩家手機會看到排名
- 線上歌單已合併詩歌、粵語 / 國語流行曲和嚴格年代歌單；80年代、90年代、00後、最近15年只收有年份證據的歌，其他流行曲會保留在全部歌單並標記為「年代未核實」
- 主持篩選是語言和分類 / 年代分維度相交：只揀 `粵語` 不會包括詩歌，要另外揀 `詩歌` 才會玩詩歌
- 隨機抽題只會使用已批准來源；待審來源會保留在題庫，但不會被抽中
- 題庫會存在同一部裝置嘅瀏覽器 localStorage
- `songlists/all-songlists.json` 是預設線上歌單，會把詩歌和全部流行曲一併載入；主持亦可直接載入粵語 / 國語或嚴格年代歌單
...
- `songlists/all-songlists.json`：全部歌單，3503 首
- `hymns.json`：詩歌，183 首
- `songlists/pop-80s.json`：80年代流行曲，40 首
- `songlists/pop-90s.json`：90年代流行曲，66 首
- `songlists/pop-00s.json`：00後流行曲（2000-2010），30 首
- `songlists/pop-recent-15.json`：最近15年流行曲（2011-2026），48 首
- `songlists/pop-recent-25.json`：已核實 00後 + 最近15年流行曲，78 首
- `songlists/pop-era-unverified.json`：年代未核實流行曲，3136 首
- `songlists/pop-cantonese.json`：粵語流行曲，1615 首
- `songlists/pop-mandarin.json`：國語流行曲，1705 首
- `songlists/all-cantonese.json`：全部粵語歌，1703 首
- `songlists/all-mandarin.json`：全部國語歌，1800 首
- `songlists/pop-all.json`：全部流行曲，3320 首

目前線上歌單只保留現有 `viewCount` metadata 達 50 萬或以上的歌曲；完整清理報告見 `docs/YOUTUBE_VIEW_FILTER_2026-06-12_500K.md`。語言 / 年代拆分 audit 見 `docs/SONGLIST_LANGUAGE_ERA_SPLIT_2026-06-12.md`，嚴格年代重檢見 `docs/SONGLIST_ERA_AUDIT_2026-06-18.md`。
