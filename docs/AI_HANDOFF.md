...
- 介面已做兩輪美化。最新 cache version 是 `home-fellowship-ui-1`。三個入口頁都載入 `assets/worship-crest.svg`；背景和遮罩使用 `assets/fellowship-pattern.svg`、`assets/home-fellowship-scene.svg`、`assets/warm-fabric-pattern.svg`。本機 `server.js` 已加入 `.svg` MIME type。
- 最新美術方向是「都會團契的家」：城市窗景、暖燈、木桌、詩歌本、杯和植物。前台遮罩仍必須是實色，不可改回半透明。
