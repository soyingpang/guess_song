...
2026-05-31 已加入 7 秒手機延遲補償：主持端用 `REMOTE_AUDIO_LATENCY_MS = 7000` 保留遠距答題 grace window，手機端用 `REMOTE_AUDIO_COUNTDOWN_DELAY_MS = 7000` 令倒數約 7 秒後才開始扣。player state 新增 `remoteAudioDelayMs`、`remotePlayEndsAt`、`answerOpenUntil`。最新 cache version 是 `mobile-delay-1`。

2026-06-01 已開始手機 premium app UI 改造：`player.html` 新增頂部 app bar、狀態 pill、設定 modal、動態背景光層和更 app-like 的主答題面板；`player.js` 新增手機 UI 設定 localStorage、動態特效 / 觸感 / 精簡畫面控制、haptic feedback 和 body 狀態 class。手機入口 cache version 是 `premium-mobile-1`，主持 / 投影入口仍是 `mobile-delay-1`。
